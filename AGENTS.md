...
- Триггер: push в `main` и ручной запуск (`workflow_dispatch`, можно на любой
  ветке: `gh workflow run prerelease-pyinstaller.yml --ref <ветка>`).
- Публикует/пересоздаёт пре-релиз с тегом `v0.0` («Предрелизная сборка v0.0»),
  предыдущий `v0.0` удаляется. Это тестовая сборка последнего запуска; тег `v0.0`
...
- Основная ветка `main`; изменения делаются в feature-ветках
  (`feature/<тема>`). Пре-релизная сборка для ручного теста на Windows
  собирается при пуше в `main`; сборку из ветки до слияния можно запустить
  вручную (см. «Пре-релиз»).
