...
4. **Токены Jira.** На диске — `dpapi:<base64>` (привязка к учётной записи
   Windows + энтропия приложения). В памяти и в UI — открытый текст, это
   осознанно (нужен для `Authorization: Bearer`). Открытый токен в файле
   (legacy) принимается и перешифровывается при загрузке; недешифруемый —
   обнуляется с предупреждением, файл при этом *не* считается битым.
...
- Автор коммитов — `iljyxa <ilja.fedorov@gmail.com>` (задан в глобальном
  `~/.gitconfig`).
